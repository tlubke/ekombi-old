...
function BeatClock:tick(dev_id)
  ...
  if self.midi and self.send and self.current_ticks % 80 == 0 then
    for id, device in pairs(midi.devices) do
      if id ~= dev_id then
        wait(this.offset)
        device:send({248})
...
function BeatClock:offset_change(ms)
  self.offset = offset
end
...
function BeatClock:add_clock_params()
  ...
  params:add_number("offset_ms", "offset", 0, 2500, self.offset)
end
